...
export function useEvmContractNFTs({ address, chain, format, limit, totalRanges, range, cursor, normalizeMetadata, mediaItems }: UseEvmContractNFTsParams = {}, queryOptions: UseEvmContractNFTsQueryOptions = {}) {
  ...
  const queryKey: [string, Partial<GetContractNFTsRequest>] = useMemo(() => {
    return [
      ...
      {
        address, chain, format, limit, totalRanges, range, cursor, normalizeMetadata, mediaItems
      },
    ];
  }, [address, chain, format, limit, totalRanges, range, cursor, normalizeMetadata, mediaItems]);
