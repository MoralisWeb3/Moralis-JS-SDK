...
export function useEvmNFTOwners({ address, chain, format, limit, cursor, normalizeMetadata, mediaItems }: UseEvmNFTOwnersParams = {}, queryOptions: UseEvmNFTOwnersQueryOptions = {}) {
  ...
  const queryKey: [string, Partial<GetNFTOwnersRequest>] = useMemo(() => {
    return [
      ...
      {
        address, chain, format, limit, cursor, normalizeMetadata, mediaItems
      },
    ];
  }, [address, chain, format, limit, cursor, normalizeMetadata, mediaItems]);
