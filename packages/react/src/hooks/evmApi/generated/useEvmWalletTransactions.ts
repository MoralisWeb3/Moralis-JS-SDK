...
export function useEvmWalletTransactions({ address, chain, fromBlock, toBlock, fromDate, toDate, cursor, limit, include }: UseEvmWalletTransactionsParams = {}, queryOptions: UseEvmWalletTransactionsQueryOptions = {}) {
  ...
  const queryKey: [string, Partial<GetWalletTransactionsRequest>] = useMemo(() => {
    return [
      ...
      {
        address, chain, fromBlock, toBlock, fromDate, toDate, cursor, limit, include
      },
    ];
  }, [address, chain, fromBlock, toBlock, fromDate, toDate, cursor, limit, include]);
