...
type RequestQuery = Pick<GetContractEventsJSONRequest, | 'chain'| 'fromBlock'| 'toBlock'| 'fromDate'| 'toDate'| 'topic'| 'offset'| 'limit'>
...
export const evmGetContractEventsResolver  = async (
  ...
  try {
    const data = await new PaginatedOperationResolver(getContractEventsOperation, Moralis.EvmApi.baseUrl, Moralis.Core).fetch(
