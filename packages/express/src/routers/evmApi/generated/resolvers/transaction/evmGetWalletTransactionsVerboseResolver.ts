...
type RequestQuery = Pick<GetWalletTransactionsVerboseJSONRequest, | 'chain'| 'fromBlock'| 'toBlock'| 'fromDate'| 'toDate'| 'cursor'| 'limit'>
...
export const evmGetWalletTransactionsVerboseResolver  = async (
  ...
  try {
    const data = await new PaginatedOperationResolver(getWalletTransactionsVerboseOperation, Moralis.EvmApi.baseUrl, Moralis.Core).fetch(
      getWalletTransactionsVerboseOperation.deserializeRequest({ ...req.params,  ...req.query, }, Moralis.Core),
    );
