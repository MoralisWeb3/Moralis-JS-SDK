...
type RequestQuery = Pick<RunContractFunctionJSONRequest, | 'chain'| 'functionName'>
...
export const evmRunContractFunctionResolver  = async (
  ...
  try {
    const data = await new OperationResolver(runContractFunctionOperation, Moralis.EvmApi.baseUrl, Moralis.Core).fetch(
