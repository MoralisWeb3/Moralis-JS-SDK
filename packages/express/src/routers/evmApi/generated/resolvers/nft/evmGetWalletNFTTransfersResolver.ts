import { GetWalletNFTTransfersJSONRequest, getWalletNFTTransfersOperation } from 'moralis/common-evm-utils';
...
type RequestQuery = Pick<
  ...
  'chain' | 'format' | 'direction' | 'fromBlock' | 'toBlock' | 'limit' | 'cursor'
>;
...
export const evmGetWalletNFTTransfersResolver = async (
  ...
  try {
    const { raw } = await new OperationResolver(
      ...
      Moralis.Core,
    ).fetch(getWalletNFTTransfersOperation.deserializeRequest({ ...req.params, ...req.query }, Moralis.Core));
