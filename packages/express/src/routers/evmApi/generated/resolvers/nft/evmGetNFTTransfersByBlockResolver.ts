import { GetNFTTransfersByBlockJSONRequest, getNFTTransfersByBlockOperation } from 'moralis/common-evm-utils';
...
export const evmGetNFTTransfersByBlockResolver = async (
  ...
  try {
    const { raw } = await new OperationResolver(
      ...
      Moralis.Core,
    ).fetch(getNFTTransfersByBlockOperation.deserializeRequest({ ...req.params, ...req.query }, Moralis.Core));
