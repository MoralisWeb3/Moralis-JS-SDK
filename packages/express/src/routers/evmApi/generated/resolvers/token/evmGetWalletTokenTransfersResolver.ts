import { GetWalletTokenTransfersJSONRequest, getWalletTokenTransfersOperation } from 'moralis/common-evm-utils';
...
type RequestQuery = Pick<
  ...
  'chain' | 'subdomain' | 'fromBlock' | 'toBlock' | 'fromDate' | 'toDate' | 'limit' | 'cursor'
>;
...
export const evmGetWalletTokenTransfersResolver = async (
  ...
  try {
    const { raw } = await new OperationResolver(
      ...
      Moralis.Core,
    ).fetch(getWalletTokenTransfersOperation.deserializeRequest({ ...req.params, ...req.query }, Moralis.Core));
