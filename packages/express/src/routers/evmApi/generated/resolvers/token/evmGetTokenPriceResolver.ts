...
type RequestQuery = Pick<GetTokenPriceJSONRequest, | 'chain'| 'exchange'| 'toBlock'>
...
export const evmGetTokenPriceResolver  = async (
  ...
  try {
    const data = await new OperationResolver(getTokenPriceOperation, Moralis.EvmApi.baseUrl, Moralis.Core).fetch(
