...
const solanaProxyRouter = express.Router();

const descriptors = Moralis.SolApi.endpoints.getDescriptors();
const rateLimiter = new RateLimiter(2, 30);

for (const descriptor of descriptors) {
  const urlPattern = descriptor.urlPattern.replace(/\{/g, ':').replace(/\}/g, '');
  solanaProxyRouter.route(urlPattern)[descriptor.method](async (req, res, next) => {
...
export default solanaProxyRouter;
